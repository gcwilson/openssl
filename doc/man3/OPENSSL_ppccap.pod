...
represented by bits in the OPENSSL_ppccap_P variable. When libcrypto initializes,
it stores the results returned by PowerPC CPU capabilities detection logic in
OPENSSL_ppccap_P. The CPU capabilities detection methods are OS-dependent and use
a combination of information gathered by the kernel during boot and probe
...
OPENSSL_ppccap_P. The ppc_arch.h header file states that, "Flags' usage can
appear ambiguous, because they are set rather to reflect OpenSSL performance
preferences than actual processor capabilities."
